...
class RNN:
    ...
    def softmax(self, x):
        p = np.exp(x - np.max(x))  # for numerical stability
        return p / np.sum(p)
    ...
    def forward(self, inputs, hprev):
        ...
        for t in range(len(inputs)):
            xs[t] = np.zeros((self.vocab_size, 1))  # Fix the shape here
    ...
    def backward(self, xs, hs, ps, targets):
        ...
        for t in reversed(range(len(xs))):
            ...
            dhrec = (1 - hs[t] * hs[t]) * dh  # derivative of tanh
            db += dhrec
            # calculate dU and dW
